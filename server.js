 ...
 */
const zipCodeRanges = {
    // Estados Unidos - Rangos de ZIP por estado
    us: {
...
/**
 * Función para generar un código postal basado en el estado/región
 */
function generateZipForState(country, state) {
    const countryRanges = zipCodeRanges[country];
    
    if (!countryRanges || !state) {
        return null;
    }
    
    // Buscar el estado (normalizar el nombre)
    const normalizedState = state.trim();
    const ranges = countryRanges[normalizedState];
    
    if (ranges && ranges.length > 0) {
        // Seleccionar un prefijo aleatorio del estado
        const prefix = ranges[Math.floor(Math.random() * ranges.length)];
        
        // Completar con dígitos aleatorios según el país
